...
- [NLP using sequential models](#nlp-using-sequential-models)
  - [Table of contents](#table-of-contents)
  - [Neural Networks](#neural-networks)
  - [Neural Networks layers](#neural-networks-layers)
  - [Traditional language models](#traditional-language-models)
    - [N-grams](#n-grams)
      - [Limitations of N-gram](#limitations-of-n-gram)
  - [Recurrent neural networks (RNN)](#recurrent-neural-networks-rnn)
    - [Why is RNN better than n-grams (what is solved)](#why-is-rnn-better-than-n-grams-what-is-solved)
    - [Applications of RNNs](#applications-of-rnns)
    - [Limitations of RNN](#limitations-of-rnn)
      - [Solutions to vanishing gradient problems](#solutions-to-vanishing-gradient-problems)
      - [Solutions to exploding gradient problems](#solutions-to-exploding-gradient-problems)
    - [Math of RNN](#math-of-rnn)
      - [Walking through math steps](#walking-through-math-steps)
      - [RNN cost function](#rnn-cost-function)
      - [Example on RNN](#example-on-rnn)
  - [Gated recurrent unites](#gated-recurrent-unites)
    - [Math of GRU](#math-of-gru)
  - [Bidirectional RNNs (BRNNs)](#bidirectional-rnns-brnns)
  - [Deep RNNs](#deep-rnns)
  - [Long short-term memory (LSTM)](#long-short-term-memory-lstm)
    - [Math of LSTM](#math-of-lstm)
  - [Named entity recognition](#named-entity-recognition)
    - [Training NER](#training-ner)
  - [Siamese networks](#siamese-networks)
    - [Architecture of Siamese network](#architecture-of-siamese-network)
...
Siamese network is type of neural network that made of 2 identical neural networks which are merged at teh end.

**Question duplicates**  

"How old are you" means the same as "What is your age" this called question duplicates.  

On the other hand "Where are you from?" does not have a near meaning to "Where are you going?" have the same first 3 words. However, the last word is different completely changes the meaning of each question.  

Comparing meaning is not simple as comparing words.  

**Application of Siamese networks in NLP:**  

- Authentication of handwritten checks.  
- Question duplicates (on platforms like Quora or stack overflow).
- Search aging queries to predict whether a new query is similar to the one that already executed.  
...
The Siamese network has Two branches where each branch has:

- Input.
- Embedding.
- LSTM (Not all Siamese networks are designed to contain LSTMs).
- Output.

The 2 branches are not separately-trained networks. 
Both branches use the exact same weight matrices. 
So if the LSTM has parameter set ${\theta}$, train one ${\theta}$ and both branches apply it.
Think of it as calling the same function twice with two different arguments, not as two different functions.  
  
Why weight sharing is essential part?  
One reason is shared weights = fewer parameters to train.  
More important reason: if question's 1 subnetwork and question's 2 subnetwork ahd different wights, they'd each be learning their own arbitrary mapping from words to vector space. The resulting vectors would live in two unrelated coordinate systems, comparing them with cosine similarity would be meaning less.

**Comparison step:**  

Each branch outputs a vector (the LSTMs encoding of the input question's meaning). Then compute cosine similarity between the two output vectors.  

$$
\hat{y} = \cos(\theta) = \frac{v_1 \cdot v_2}{\|v_1\| \|v_2\|}
$$  

**Decision rule:**

$$
\hat{y} > {\tau} ⇒ duplicate, \hat{y} ≤ {\tau} ⇒ different
$$  

where ${\tau}$ is threshold hyperparameter.  

### Loss function 

Anchor: the input question that used to compare other questions against it.  

Positive question: question that has the same meaning to the anchor.  

Negative question: question that does not have the same meaning to the anchor.  

To train the model compare the vectors that are output by each sub network using cosine similarity.  

$$
cos (v1, v2) = \frac{v_1 \cdot v_2}{\|v_1\| \|v_2\|}
$$  

S(A,P): the similarity between the anchor and the positive question. The similarity is between (-1, 1) so for similar questions (nearly identical vectors) the similarity is near 1.  

S(A,N): the similarity between the anchor and the negative question. It is near -1.  

Loss function: diff = S(A,N) - S(A,P).  

## Triplets

Triplets: group of anchors, positive and negative examples.  

Simple loss: diff = S(A,N) - S(A,P).

<img src="images/image-12.png"/>  

Increasing the hyperparameter alpha requires more optimization during training.  

- ${\alpha}$: Controls how far cos(A,P) is from cos(A,N).  
- Easy negative triplet: cos(A,N) < cos (A,P).  
- Semi-hard negative triplet: cos(A,N) < cos (A,P) < cos cos(A,N) + ${\alpha}$.  
- Hard negative triplet: cos(A,P) < cos (A,P).

### Compute the cost

To compute the cost first you prepare the batches where every every example is an anchor and its own positive (each example on the left has a similar example on the write but no other example above or below it have the same meaning). The batch size is the size of sum of the examples.  
...
$$
L(A, P, N) = max(diff +{\alpha})
$$
$$
diff = s(A, N) - s (A, P)
$$  

**Mean negative (mean_neg):** the mean negative of all the other off diagonals in a row.

**Closest negative (closest_neg):** the corresponding to the highest number in the off diagonals (The larges number).  
...
In problems like detecting author of a book or authentication of a signature there is 2 approaches.

- **classification:** If you trained a model with dataset of K classes and you need to add a new class, you need to retrain the whole model with class = K + 1 with a classifier as softmax on all of them.  
The problem that every time a new class appears you have to retrain the whole model, and if there is'nt enough new class's data the model wont have a good decision boundary for that class

- **One-shot learning:** Instead of "which K classes does this belong to" the question becomes "are these two things the same or different".  
Using a similarity function (like Siamese network + cosine similarity) you can compare any two inputs even the ones it's never seen before without retrain the whole model as the model only needs one reference  example per new class - thats why it is called one-shot.  